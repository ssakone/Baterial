...
Item {
	...
	function urlToLocalFile(url) {
		...
		path = isWindows ? path.replace(/^(file:\/{3})/,
										"") : path.replace(/^(file:\/{2})/, "")
	...
	Component {
		...
		Qaterial.AlertIconDialog {
			...
			title: root.settings
				   && root.settings.title ? root.settings.title : ""
			iconSource: root.settings
						&& root.settings.iconSource ? root.settings.iconSource : ""
			iconColor: root.settings
					   && root.settings.iconColor ? root.settings.iconColor : Qaterial.Style.accentColor
			standardButtons: root.settings
							 && root.settings.standardButtons ? root.settings.standardButtons : Dialog.NoButton
	...
	Component {
		...
		Qaterial.TextFieldDialog {
			...
			title: root.settings
				   && root.settings.title ? root.settings.title : ""
			textTitle: root.settings
					   && root.settings.textTitle ? root.settings.textTitle : ""
			placeholderText: root.settings
							 && root.settings.placeholderText ? root.settings.placeholderText : ""
			helperText: root.settings
						&& root.settings.helperText ? root.settings.helperText : ""
			validator: root.settings
					   && root.settings.validator ? root.settings.validator : null
			inputMethodHints: root.settings
							  && root.settings.inputMethodHints ? root.settings.inputMethodHints : 0
			maximumLengthCount: root.settings
								&& root.settings.maximumLengthCount ? root.settings.maximumLengthCount : 32767
			selectAllText: root.settings
						   && root.settings.selectAllText ? root.settings.selectAllText : false
			errorText: root.settings
					   && root.settings.errorText ? root.settings.errorText : ""
			echoMode: root.settings
					  && root.settings.echoMode ? root.settings.echoMode : TextInput.Normal
			standardButtons: root.settings
							 && root.settings.standardButtons ? root.settings.standardButtons : Dialog.NoButton
			dialogImplicitWidth: root.settings
								 && root.settings.width ? root.settings.width : Qaterial.Style.dialog.implicitWidth
	...
	Component {
		...
		Qaterial.RadioDialog {
			title: (root.settings
					&& root.settings.title) ? root.settings.title : ""
			currentIndex: (root.settings && root.settings.currentIndex
						   !== undefined) ? root.settings.currentIndex : -1

			model: (root.settings
					&& root.settings.model) ? root.settings.model : null
			delegate: (root.settings
					   && root.settings.delegate) ? root.settings.delegate : defaultDelegate
			...
			Component.onCompleted: {
				if (root.settings
						&& (root.settings.standardButtons !== undefined))
	...
	Component {
		...
		QLab.FileDialog {
			...
			defaultSuffix: (root.settings
							&& root.settings.defaultSuffix) ? root.settings.defaultSuffix : ""
			nameFilters: (root.settings
						  && root.settings.nameFilters) ? root.settings.nameFilters : undefined
			fileMode: (root.settings
					   && root.settings.fileMode) ? root.settings.fileMode : 0
			options: (root.settings
					  && root.settings.options) ? root.settings.options : 0

			onAccepted: function () {
				if (fileDialog.fileMode === QLab.FileDialog.OpenFiles) {
					if (root.settings && root.settings.onAccepted) {
						let filePaths = []
						fileDialog.files.forEach(element => filePaths.push(
													 root.urlToLocalFile(
														 element)))
					...
					root.settings.onAccepted(
								root.urlToLocalFile(
									fileDialog.currentFile.toString()))
				}
			}
	...
	function _show(settings, comp) {
		...
		if (settings && settings.context) {
			if (settings.context instanceof QtObject) {
				...
				Qaterial.Logger.warn(
							`${settings.context} isn't an instance of QtObject. It can't be used as a dialog context.`)
				root.context = null
			}
		} else {
			root.context = null
	...
	function openWithSettings(settings) {
		Qaterial.Logger.warn(
					"'DialogLoader.openWithSettings' is deprecated, consider moving to 'showDialog'")
		showDialog(settings)
	}
